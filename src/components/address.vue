    ...
    <div>
        ...
        <el-card class="box-card">
            ...
            <el-row :gutter="20">
                ...
                <el-col :span="6"> <div class="grid-content bg-purple"> <el-input placeholder="请输入地址"  class="input-with-select" v-model="queryInfo.key.address" clearable @clear="getlist">

                </el-input></div></el-col>
                <el-col :span="6"> <div class="grid-content bg-purple"> <el-input placeholder="请输入邮编"  class="input-with-select" v-model="queryInfo.key.postalcode" clearable @clear="getlist">
            ...
            <el-row :gutter="20">
                ...
                <el-col :span="6"><div class="grid-content bg-purple" ><el-button type="primary" @click="dialogVisible=true" size="mini">添加地址</el-button></div></el-col>
            </el-row>
    ...
    <el-button type="primary"  @click="adduser">确 定</el-button>
        ...
        >
            <el-form label-width="80px" :model="xgyh" :rules="xgyhformrules1" ref="xgyhformref">
                <el-form-item label="地址"  prop="address">
                    <el-input v-model="xgyh.address" ></el-input>
                </el-form-item>
                <el-form-item label="邮编" prop="postalcode">
                    <el-input v-model="xgyh.postalcode" ></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
...
<script>
    export  default {
        data(){
            return{
                ...
                rules:{
                    ...
                    postalcode:[{ required: true, message: '请输入邮编名称', trigger: 'blur' },{ min: 6, max: 6, message: '输入合法邮编号', trigger: 'blur' }]
                },
                userlist:[],
                total:0,
                userVisible:false,
        ...
        methods:{
            async getlist(){
                const {data:res}=  await this.$http.post("address/all",this.queryInfo)
            ...
            adduser(){
                this.$refs.addformref.validate(async valid=> {
                    if (!valid) return
                    const res = await this.$http.post("address/save", this.addform)
                    console.log(res)
                    this.getlist()
            ...
            async  xgyhxx(v1){
                ...
                const res=   await   this.$http.get("address/find/"+v1)
                this.xgyh=res.data
            ...
            xhyhclose(){
              ...
              xguser(){
                this.$refs.xgyhformref.validate(async valid=> {
                    if (!valid) return
                    const res = await this.$http.post("address/updateCustomer", {
                        id: this.xgyh.id,
                        address: this.xgyh.address,
                        postalcode: this.xgyh.postalcode
                    })
            ...
            async deleteuser(v1){
                ...
                const res= await this.$http.post(`address/updateStateId/${v1}/0`)
                console.log(res)
            ...
            async star(v1){
                const res= await this.$http.post(`address/updateStateId/${v1}/1`)
                console.log(res)
                //if (res.meta.status!==200) return this.$message.error("删除失败")
            ...
            async   plsc(){
                ...
                const res=await  this.$http.post("address/updateStateIds",this.newarr)
                this.getlist()
            }
        }
