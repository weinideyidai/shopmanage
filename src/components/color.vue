<template>
    <div>
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>颜色信息</el-breadcrumb-item>
        </el-breadcrumb>
        <el-card class="box-card">

            <el-row :gutter="20">

                <el-col :span="9"> <div class="grid-content bg-purple"> <el-input placeholder="请输入颜色名"  class="input-with-select" v-model="queryInfo.key.name" clearable @clear="getlist">
            ...
            <el-button type="danger" size="mini" @click="plsc">批量禁用</el-button>
                ...
                <el-col :span="2"><div class="grid-content bg-purple"><el-button type="primary" @click="colordialogVisible=true" size="mini">添加颜色</el-button></div></el-col>
            </el-row>
            <el-table
                    :data="userlist"
        ...
        <el-dialog
            ...
            <el-form label-width="80px" :model="addform" :rules="colorrules" ref="addformref">
                <el-form-item label="颜色名" prop="name" :error="msg">
                    <el-input v-model="addform.name" @blur="check"></el-input>
                </el-form-item>
            </el-form>
    ...
    <el-button type="primary"  @click="adduser">确 定</el-button>
        ...
        <el-dialog
                ...
                @close="xhyhclose"
        >
            <el-form label-width="80px" :model="xgyh" :rules="xgcolorrules" ref="xgyhformref">
    ...
    export  default {
        data(){
            return{
                ...
                newarr:[],
                value:"",
                colorrules:{name:[ { required: true, message: '请输入颜色', trigger: 'blur' } ]},
                xgcolorrules:{name:[ { required: true, message: '请输入颜色', trigger: 'blur' } ]},
                msg:""
            }
        },
        created() {
        ...
        methods:{
            async getlist(){
                const {data:res}=  await this.$http.post("colour/all",this.queryInfo)
            ...
            adduser(){
                this.$refs.addformref.validate(async valid=> {
                    ...
                    const res = await this.$http.post("colour/save", this.addform)
                    console.log(res)
                    this.getlist()
            ...
            async  xgyhxx(v1){
                const res=   await   this.$http.get("colour/find/"+v1)
                this.xgyh=res.data
            ...
            xguser(){
                this.$refs.xgyhformref.validate(async valid=> {
                    if (!valid) return this.$message.error("请填写必填信息")
                    const res = await this.$http.post("colour/updateColour", {
                        id: this.xgyh.id,
                        name: this.xgyh.name
                    })
            ...
            async deleteuser(v1){
                ...
                const res= await this.$http.post(`colour/updateStateId/${v1}/0`)
                console.log(res)
            ...
            async star(v1){
                const res= await this.$http.post(`colour/updateStateId/${v1}/1`)
                console.log(res)
                //if (res.meta.status!==200) return this.$message.error("删除失败")
            ...
            async   plsc(){
                ...
                const res=await  this.$http.post("colour/updateStateIds",this.newarr)
                this.getlist()
            },
            check(){
                this.msg="55555"
