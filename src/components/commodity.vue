<template>
    <div>
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>商品类型信息</el-breadcrumb-item>
        </el-breadcrumb>
        <el-card class="box-card">

            <el-row :gutter="20">

                <el-col :span="9"> <div class="grid-content bg-purple"> <el-input placeholder="请输入商品类型名"  class="input-with-select" v-model="queryInfo.key.name" clearable @clear="getlist">
            ...
            <el-button type="danger" size="mini" @click="plsc">批量禁用</el-button>
                ...
                <el-col :span="2"><div class="grid-content bg-purple"><el-button type="primary" @click="dialogVisible=true" size="mini">添加类型</el-button></div></el-col>
            </el-row>
            <el-table
                    :data="userlist"
        ...
        <el-dialog
            ...
            <el-form label-width="80px" :model="addform" :rules="jj" ref="addformref2">
    ...
    <el-button type="primary"  @click="adduser">确 定</el-button>
        ...
        <el-dialog
                ...
                @close="xhyhclose"
        >
            <el-form label-width="80px" :model="xgyh" :rules="xgbrands" ref="xgyhformref">
                <el-form-item label="商品类型"  prop="name">
    ...
    export  default {
        data(){
            return{
                ...
                newarr:[],
                value:"",
                jj:{ name:[{ required: true, message: '请输入商品类型名称', trigger: 'blur' }]},
                xgbrands:{ name:[{ required: true, message: '请输入商品类型名称', trigger: 'blur' }]}
            }
        },
        created() {
            this.getlist()
        ...
        methods:{
            async getlist(){
                const {data:res}=  await this.$http.post("goodType/all",this.queryInfo)
            ...
            adduser(){
                this.$refs.addformref2.validate(async valid=> {
                    ...
                    const res = await this.$http.post("goodType/save", this.addform)
                    console.log(res)
                    this.getlist()
            ...
            async  xgyhxx(v1){
                ...
                const res=   await   this.$http.get("goodType/find/"+v1)
                this.xgyh=res.data
            ...
            xguser(){
                this.$refs.xgyhformref.validate(async valid=> {
                    if (!valid) return this.$message.error("请填写必填信息")
                    const res = await this.$http.post("goodType/updateGoodType", {
                        id: this.xgyh.id,
                        name: this.xgyh.name
                    })
            ...
            async deleteuser(v1){
                ...
                const res= await this.$http.post(`goodType/updateStateId/${v1}/0`)
                console.log(res)
            ...
            async star(v1){
                const res= await this.$http.post(`goodType/updateStateId/${v1}/1`)
                console.log(res)
                //if (res.meta.status!==200) return this.$message.error("删除失败")
            ...
            async   plsc(){
                ...
                const res=await  this.$http.post("goodType/updateStateIds",this.newarr)
                this.getlist()
            }
        }
